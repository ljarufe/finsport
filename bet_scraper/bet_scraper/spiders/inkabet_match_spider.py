...
class InkabetMatchSpider(Spider):
    ...
    custom_settings = (
        {
            "ITEM_PIPELINES": {"bet_scraper.pipelines.MatchPipeline": 300},
        },
    )
    ...
    def parse_items(self, date_selector):
        ...
        for item in items:
            url = urljoin(
                InkabetMatchSpider.start_urls[0],
            ...
            try:
                country, league = selector.css(
                    ".osg-coupon__breadcrumbs a::text"
                ).getall()[2:4]
            ...
            try:
                local_factor = float(
                    selector.xpath(
                        ...
                        "div[2]/div[2]/div/div[1]/div/div[2]/div/text()"
                    ).get()
                )
                draw_factor = float(
                    selector.xpath(
                        '//*[@id="osg-app"]/div/div[1]/div/div[2]/div[2]/div[2]/'
                        "div[2]/div[2]/div/div[2]/div/div[2]/div/text()"
                    ).get()
                )
                visitor_factor = float(
                    selector.xpath(
                        '//*[@id="osg-app"]/div/div[1]/div/div[2]/div[2]/div[2]/'
                        "div[2]/div[2]/div/div[3]/div/div[2]/div/text()"
                    ).get()
                )
            except TypeError:
                continue
