...
def initialize_primary_series():
    ...
    competition_ids = list(
        Competition.objects.filter(enabled=True)
        .order_by("id")
...
@transaction.atomic
def _clear_current_snapshot(series):
    locked = CapitalLongitudinalSeries.objects.select_for_update().get(pk=series.pk)
    if locked.current_snapshot_id is not None:
        locked.current_snapshot = None
        locked.save(update_fields=["current_snapshot", "modified"])
...
def recompute_longitudinal_capital(*, series=None, pipeline_run_id=None):
    try:
        ...
        logical_identity = _logical_identity(series, basis.input_hash)
        with transaction.atomic():
            locked = CapitalLongitudinalSeries.objects.select_for_update().get(
                pk=series.pk
            )
            current = locked.current_snapshot
            if current and current.logical_identity == logical_identity:
                return _result(
                    locked,
                    basis,
                    status="NO_WORK",
        ...
        existing = CapitalExperiment.objects.filter(
        ...
        if existing:
            with transaction.atomic():
                ...
                locked.current_snapshot = existing
                locked.save(update_fields=["current_snapshot", "modified"])
            return _result(
                series,
                basis,
        ...
        def policy_failure(error, experiment, run):
            ...
            emit_event(
                ...
                human_summary="A longitudinal Capital policy arm failed unexpectedly.",
                exception=error,
        ...
        with transaction.atomic():
            ...
            locked.current_snapshot = experiment
            locked.save(update_fields=["current_snapshot", "modified"])
        if not experiment.policy_runs.filter(
        ...
        context = _failure_context(
            ...
            basis if "basis" in locals() else None,
        )
        if isinstance(series, CapitalLongitudinalSeries):
            try:
                _clear_current_snapshot(series)
