    ...
    def get_league(cls, league_name, country, bet_page):
        ...
        if country:
            related_league = cls.objects.filter(
                name__unaccent__iexact=league_name,
                league__country=country,
                bet_page=bet_page,
            )
...
class Match(TimeStampedModel):
    ...
    def __str__(self):
        return f"{self.local_team.name} - {self.visitor_team.name} ({self.league}), {self.start_datetime}"
    ...
    def get_team_difference_score(self):
        return Match.MAX_SCORE_DIFFERENCE * (
            1 - abs(self.local_factor - self.visitor_factor) / Match.TEAM_DIFFERENCE
        )
    ...
    def get_league_score(self):
        ...
        return (
            2
            * (self.league.draw_percentage - Match.MIN_SCORE_LEAGUE)
            / (limit["draw_percentage__max"] - Match.MIN_SCORE_LEAGUE)
        )

    def get_match_name(self):
        return f"{self.local_team.name} - {self.visitor_team.name}"

    def get_match_score(self):
        if self.local_score is not None:
            return f"{self.local_score} - {self.visitor_score}"
        return "-"

    def get_logger_info(self):
        return f"{self.id}: {self.local_team.name} - {self.visitor_team.name}, {self.start_datetime}"
    ...
    def check_rules(self):
        ...
        if (
            self.local_factor < Match.MIN_PER_TEAM
            or self.visitor_factor < Match.MIN_PER_TEAM
        ):
