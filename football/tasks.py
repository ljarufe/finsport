...
def get_livescore_matches():
    ...
    response = requests.get(
        f"https://prod-cdn-mev-api.livescore.com/v1/api/app/date/soccer/{today}/-5?locale=en"
    )
    for stage in response.json().get("Stages", []):
        league_name = stage["Snm"]
        country = stage["CnmT"]
        league = League.objects.get_league(
            league_name, country, field_name="name_en", language="en"
        )
        if league:
            for match_data in stage.get("Events", []):
                ...
                state = match_data.get(
                    "Eps", None
                )  # FT: Full Time, HT: Half Time, NS: Not Started, etc.
    ...
    return stages  # noqa: F821 - legacy return contract is intentionally deferred
